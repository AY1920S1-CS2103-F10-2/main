...
****
* `e/EXPIRY_DATE` must follow the format `dd.MM.yyyy` or `dd/MM/yyyy` or `dd-MM-yyyy`.
* `a/AMOUNT` must have a magnitude i.e. measurable quantity. Amount unit is not mandatory. Magnitude and unit can be separated by a space.
...
==== Sort the grocery list: `glist sort` (Coming in v2.0)
...
==== Filter: `glist filter` (Coming in v2.0)

Returns a list of food items that matches the specified tag. +
Format: `glist filter t/TAG`

==== Locating items by name: `glist find` (Coming in v2.0)
...
Returns any grocery item which contains any of the names `milo`, `doughnut`, or `roasted`
